...
@dp.message_handler(content_types=['audio', 'photo', 'voice', 'video', 'document', 'text',
                                   'location', 'contact', 'sticker', 'poll',
                                   'animation','game', 'video_note', 'dice', 'venue'])
    ...
    if (str(message.chat.id), ) in db.get_chats():
        if user_id not in admins and not await is_admin(message):
            ...
            if "/unban" in message.text:
                ...
                await message.answer(f'somethi')